...
public class FrmDanhSachHoaDon extends JFrame implements ActionListener, MouseListener, KeyListener {
	...
	public FrmDanhSachHoaDon(JFrame frm) {
		...
		pSapXep.setBorder(new TitledBorder(new LineBorder(new Color(114, 23, 153), 1, true), "Chọn thời gian",
		...
		String col[] = { "Mã hóa đơn", "Mã khách hàng", "Tên khách hàng", "Mã nhân viên", "Tên nhân viên", "Ngày lập",
				"Phụ thu" };
	...
	public void loadTableHoaDon(ArrayList<HoaDon> lsHD) {
		...
		for (HoaDon hd : lsHD) {
			...
			modelHoaDon.addRow(new Object[] { hd.getMaHoaDon(), kh.getMaKhangHang(), kh.getTenKH(), nv.getMaNhanVien(),
					nv.getTenNhanVien(), sf.format(hd.getNgayLap()), hd.getPhuThu() });
	...
	public double tinhTienThue(double giaPhong, HoaDon hd) {
		int gioVao = hd.getGioVao().getHours(), phutVao = hd.getGioVao().getMinutes();
		int gioRa = hd.getGioRa().getHours(), phutRa = hd.getGioRa().getMinutes();
	...
	public void loadTimKiem() {
		...
		thongTin = thongTin.toUpperCase(); // chuyển toàn bộ chuỗi thành chữ in hoa hết để tìm, tránh trường hợp người
											// dùng nhập chữ thường chữ hoa
		...
		if (regexDao.regexTimDSHD(txtTim)) { // nếu check thông tin nhập vô, thỏa điều kiện là mã hoặc tên
			...
			if (thongTin.matches(regexMaNV)) { // regex check lần nữa để phân biệt đc nó là mã, nếu là mã thì lấy danh
												// sách hóa đơn theo mã
				lsHD = daoHD.getHDTheoMaNV(thongTin);

			} else if (thongTin.matches(regexTenKH)) { // nếu check ko phải là mã thì nó check có phải tên hay ko,nếu là
														// tên thì get dsHD theo tên
				lsHD = daoHD.getHDTheoTenKH(thongTin);
			}
			if (lsHD.size() == 0) // vì ds hóa đơn lấy về có thể bị rỗng, nên phải kiểm tra lại, kích thước nó = 0
									// thì xuất thông báo ko tìm thấy
	...
	public void loadTableCTHD() {
		...
		for (CTHD ct : lsCTHD) {
			...
			modelCTHD.addRow(new Object[] { mh.getMaMatHang(), mh.getTenMatHang(), loaiMH.getTenLoaiMatHang(),
					ct.getSoLuong(), dfTable.format(mh.getGiaMatHang()), dfTable.format(tongTien) });
		}
		loadThanhTien(hd);
