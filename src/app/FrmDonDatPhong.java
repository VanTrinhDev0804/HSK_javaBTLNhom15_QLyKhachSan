...
public class FrmDonDatPhong extends JPanel implements ActionListener, FocusListener {
	...
	private JLabel lblTim, lblTenKH, lblLoaiKH, lblNgayDen, lblSDT, lblGioDen, lblTinhTrangDDP, lblDiaChi, lblChonPhong,
			lblBackGround;
	...
	public FrmDonDatPhong(String sHeaderTenNV, String sHeaderMaNV, Date dNgayHienTai) {
		...
		/**
		 * Nhập thông tin đơn đặt phòng mới Panel pNhapThongTin
		 */
		...
		/**
		 * Tìm kiếm đơn đặt phòng theo họ tên và sđt khách hàng. Tìm khách hàng theo
		 * sđt. Label lblTim JTextField txtTim Sự kiện placeholder tìm DDP:
		 * FocusListener Nút tìm DDP và KH Icon iconTim
		 */
		...
		/**
		 * Bảng chứa các thông tin phòng JScrollPane scrollPaneChonPhong String col[]:
		 * tên các cột JTable tblPhong: nội dung của bảng JTableHeader tbHeaderPhong:
		 * định dạng khung các tên cột
		 */
		...
		JScrollPane scrollPaneChonPhong = new JScrollPane(tblPhong, JScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED,
		...
		/**
		 * Thêm 1 DDP vào danh sách bảng DDP Nút thêm DDP JButton btnThemDDP Icon
		 * iconThemDDP
		 */
		...
		/**
		 * Sửa thông tin DDP Nút sửa DDP JButton btnSuaDDP Icon iconSuaDDP
		 */
		...
		/**
		 * Làm mới: xóa trắng các text, xóa tất cả nội dung trong bảng DDP, đặt mặc định
		 * các combobox, bỏ chọn checkbox và các radiobutton Nút làm mới JButton
		 * btnLamMoiDDP Icon iconLamMoiDDP
		 */
		...
		/**
		 * Nhóm các radiobutton ButtonGroup bg
		 */
		bg = new ButtonGroup();

		/**
		 * Bảng chứa các thông tin đơn đặt phòng JScrollPane scrollPaneDDP String col[]:
		 * tên các cột JTable tblDDP: nội dung của bảng JTableHeader tbHeaderDDP: định
		 * dạng khung các tên cột
		 */
		...
		String colDDP[] = { "Mã DDP", "Mã phòng", "Tên KH", "SĐT", "Ngày đến", "Giờ đến", "Tên NV lập", "Ngày lập",
				"Trạng thái DDP" };
	...
	/**
	 * Hiển thị danh sách thông tin phòng trống và đã đặt gồm: mã phòng, loại phòng,
	 * giá phòng, tình trạng phòng
	 * 
	 * @param p
	 */
	private void loadDSPhongTrongVaDaDat(Phong p) {
		...
		for (Phong infoP : lstP) {
			...
			modelPhong
					.addRow(new Object[] { infoP.getMaPhong(), lp.getTenLoaiPhong(), gia, infoP.getTinhTrangPhong() });
		}
		changeColorRow(tblPhong);
	}

	/**
	 * Hiển thị danh sách thông tin đơn đặt phòng, không hiển thị đơn đã hủy, thông
	 * tin gồm: mã ĐĐP, mã phòng, tên KH, sđt, ngày đến, giờ đến, tên NV lập, ngày
	 * lập, trạng thái ĐĐP
	 * 
	 * @param ddp
	 */
	private void loadDanhSachDDP(DonDatPhong ddp) {
		...
		for (DonDatPhong infoDDP : lstDDP) {
			...
			modelDDP.addRow(new Object[] { infoDDP.getMaDDP(), infoDDP.getPhong().getMaPhong(), kh.getTenKH(),
					kh.getSdt(), dfNgay.format(infoDDP.getNgayDen()), dfHienGio.format(infoDDP.getGioDen()),
					nv.getTenNhanVien(), dfNgay.format(infoDDP.getNgayLap()), infoDDP.getTrangThaiDDP() });
		}
	...
	private void loadDDPTheoSdtKH(ArrayList<DonDatPhong> list) {
		...
		for (DonDatPhong infoDDP : lstDDP) {
			NhanVien nv = daoNhanVien.getMaVaSdtNVChoDDP(infoDDP.getNhanVien().getMaNhanVien());
			modelDDP.addRow(new Object[] { infoDDP.getMaDDP(), infoDDP.getPhong().getMaPhong(), khachHang.getTenKH(),
					khachHang.getSdt(), dfNgay.format(infoDDP.getNgayDen()), dfHienGio.format(infoDDP.getGioDen()),
					nv.getTenNhanVien(), dfNgay.format(infoDDP.getNgayLap()), infoDDP.getTrangThaiDDP() });
		}
	}
	...
	private void choose1DDP() {
		...
		if ((selectedRow >= 0 && tblPhong.getSelectedRow() == -1)
				|| (selectedRow >= 0 && tblPhong.getSelectedRow() != -1)) {
	...
	/**
	 * Sự kiện tìm kiếm thông tin ĐĐP theo họ tên và sđt của KH Sự kiện tìm kiếm
	 * thông tin KH theo sđt của KH
	 */
	@SuppressWarnings("deprecation")
	private void findKHVaDDP() {
		...
		if (regex.regexTimDDPTheoKH(txtTim)) {
			if (input.matches(regexSDT)) {
				...
				if (lstKH1.size() == 0) {
					JOptionPane.showMessageDialog(this, "Không tìm thấy thông tin tìm kiếm phù hợp!", "Thông báo",
...
//		
	...
	/**
	 * Kiểm tra các thông tin khách hàng trước khi đặt đơn Tìm kiếm KH theo sđt, nếu
	 * thông tin KH đã có trong dữ liệu thì hiện thông tin KH lên và đặt đơn Nếu KH
	 * mới, tìm không có thông tin thì thêm 1 KH mới và dữ liệu và được đặt đơn
	 */
			...
			if (chonPhong >= 0) {
				...
				if (regex.regexTen(txtTenKH) && regex.regexSDT(txtSDT) && regex.regexDiaChi(txtDiaChi)) {
					if (tinhTrangPhong.equals("Đã đặt"))
						JOptionPane.showMessageDialog(null, "Phòng này đã được đặt, vui lòng chọn phòng khác!",
								"Thông báo", JOptionPane.OK_OPTION);
					else {
						if (daoKhachHang.checkSdtKH(sdt)) { // kq=true thì lấy thông tin KH cũ
							// them vao data
							DonDatPhong ddp = new DonDatPhong(phatSinhMaDDP, ngayLap, trangThaiDDP, ngayDen, gioDen, kh,
									nv, p);
							try {
								if (trangThaiDDP.equals("Chờ xác nhận")) {
									...
									resetAll();
									JOptionPane.showMessageDialog(this, "Thêm đơn đặt phòng thành công!", "Thông báo",
								...
								if (trangThaiDDP.equals("Hủy") || trangThaiDDP.equals("Đã trả phòng"))
									JOptionPane.showMessageDialog(this,
											"Không được thêm đơn đặt phòng có trạng thái đơn là hủy hoặc đã trả phòng!",
											"Thông báo", JOptionPane.WARNING_MESSAGE);
							} catch (SQLException e) {
								e.printStackTrace();
								JOptionPane.showMessageDialog(this, "Thêm đơn đặt phòng thất bại!", "Thông báo",
										JOptionPane.ERROR_MESSAGE);
							}
						...
						if (!daoKhachHang.checkSdtKH(sdt)) { // kq=false thì thêm KH mới
							try {
								if (trangThaiDDP.equals("Chờ xác nhận")) {
									KhachHang newKH = new KhachHang(phatSinhMaKH,
											new LoaiKH(daoLoaiKH.getMaLoaiKHTheoTen("Khách hàng thường")), hoTen, sdt,
											diaChi);
									daoKhachHang.themDanhSachKH(newKH);

									DonDatPhong ddp = new DonDatPhong(phatSinhMaDDP, ngayLap, trangThaiDDP, ngayDen,
											gioDen, newKH, nv, p);
									daoDonDatPhong.themDDP(ddp);
									daoPhong.capnhatTrangThaiPhong(maPhongChon, "Đã đặt");
				...
				JOptionPane.showMessageDialog(null, "Vui lòng chọn phòng muốn đặt!", "Thông báo",
		...
		if (chooserNgayDen.getDate() != null) {
			...
			if (ngayDen.equals(dNgayHienTai)) {
				...
				if (gio < hourNow || (gio == hourNow && phut < minNow))
					JOptionPane.showMessageDialog(this,
							"Giờ đến phải được đặt sau hoặc ngay giờ hiện tại! \nGiờ hiện tại là: "
									+ dfHienGio.format(dateNow),
							"Thông báo", JOptionPane.WARNING_MESSAGE);
			} else
				JOptionPane.showMessageDialog(this,
						"Ngày đến phải đặt trong ngày hôm nay! \nNgày hôm nay là: " + dfNgay.format(dNgayHienTai),
						"Thông báo", JOptionPane.WARNING_MESSAGE);
	...
	/**
	 * Sửa, cập nhật thông tin ĐĐP, trong giao diện quản lý ĐĐP không được sửa thông
	 * tin KH
	 * 
	 * @throws SQLException
	 */
	...
	private void updateDDP() throws SQLException { // thông tin KH trong ddp ko đc sửa
		...
		if (row >= 0) {
			int update = JOptionPane.showConfirmDialog(this, "Bạn muốn sửa thông tin đơn đặt phòng này?", "Thông báo",
			...
			if (update == JOptionPane.YES_OPTION) {
				...
				modelDDP.addRow(new Object[] { maDDP, maPhong, kh.getTenKH(), kh.getSdt(), dfNgay.format(ngayDen),
						dfHienGio.format(gioDen), nv.getTenNhanVien(), dfNgay.format(ngayLap), trangThaiDDP });
				JOptionPane.showMessageDialog(this, "Sửa thông tin đơn đặt phòng thành công!", "Thông báo",
						JOptionPane.INFORMATION_MESSAGE);
			}
		} else
			JOptionPane.showMessageDialog(this, "Vui lòng chọn thông tin đơn đặt phòng cần sửa!", "Thông báo",
					JOptionPane.WARNING_MESSAGE);
	}
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (o.equals(btnTim)) {
			if (txtTim.getText().equals("") || txtTim.getText()
					.equals("Tìm đơn đặt phòng theo họ tên và sđt khách hàng, tìm khách hàng theo sđt.")) {
				removeDanhSachDDP(modelDDP);
